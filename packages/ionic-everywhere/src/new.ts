...
import { runChecks, formatReport } from "./doctor";
...
async function resolveConfig(opts: NewOptions): Promise<ResolvedConfig> {
  ...
  if (!targetDir && !opts.yes) {
    const answer = await p.text({
      ...
      placeholder: "./my-app",
      validate: (v) =>
        v.trim().length === 0 ? "Please enter a directory" : undefined,
    });
    checkCancel(answer);
...
export async function runNew(opts: NewOptions): Promise<number> {
  ...
  if (!cfg.install) {
    ...
    p.log.message(`  ${pmRun(cfg.pm)} cap add android`);
  ...
  p.outro(
    [
      ...
      `  ${cfg.pm} run build:desktop # Windows/macOS/Linux app`,
