...
> One responsive Ionic React codebase → Web + Android + Desktop.

Scaffolds a single TypeScript project using Ionic React 9, Capacitor 8 and Electron —
write your UI once, ship it to the browser, Google Play and Windows/macOS/Linux
...
| `add <platform>` | Add Android or Desktop to an existing project |
| `doctor`         | Check environment and prerequisites           |
| `build`          | Build the CLI                                 |
| `list`           | List available templates and overlays         |
| `upgrade`        | Interactive upgrade helper                    |
| `sign`           | Signing / identity helper (macOS / Windows)   |
...
Scripts are generated for your chosen package manager (`--pm bun|npm|pnpm|yaml`) —
...
- **Actively maintained desktop bridge** — `@capawesome/capacitor-electron` (MIT) replaces
  the abandoned `@capacitor-community/electron` with the familiar `cap add / sync / run`
  workflow.
